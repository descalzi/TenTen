  ...
  const checkAndAction = React.useCallback(
    (data: SquareDataType) => {
      ...
      if (isPressFullScore(data.value)) {
        console.log(
          "disable buttons, update display score, clean pressTracker"
        );
  ...
  const handlePress = React.useCallback(
    (data: SquareDataType) => {
      ...
      if (pressTracker.length === 1) {
        ...
        if (
          pressTracker[0].row === data.row ||
          pressTracker[0].col === data.col
        ) {
          checkAndAction(data);
        }
        return;
      ...
      if (
        pressTracker[0].row === pressTracker[1].row &&
        pressTracker[1].row === data.row
      ) {
        checkAndAction(data);
        return;
      }
      if (
        pressTracker[0].col === pressTracker[1].col &&
        pressTracker[1].col === data.col
      ) {
        checkAndAction(data);
        return;
      }
...
export const PlayArea = () => {
  ...
  React.useEffect(() => {
    Array.from(Array(10).keys()).forEach((position: number) => {
      if (
        disableTracker.filter(
          (square: SquareDataType) => square.row === position + 1
        ).length === 10 &&
      ...
      if (
        disableTracker.filter(
          (square: SquareDataType) => square.col === position + 1
        ).length === 10 &&
        !completedRow.includes(position + 1)
      ) {
        setCompletedRow((prev: number[]) => prev.concat(position + 1));
