...
export const PlayArea = () => {
  ...
  React.useEffect(() => {
    Array.from(Array(10).keys()).forEach((position: number) => {
      if (
        disableTracker.filter((square: SquareDataType) => square.row === position + 1).length ===
          10 &&
      ...
      if (
        disableTracker.filter((square: SquareDataType) => square.col === position + 1).length ===
          10 &&
        !completedRow.includes(position + 1)
      ) {
        setCompletedRow((prev: number[]) => prev.concat(position + 1))
  ...
  return (
    <Layout style={playAreaStyles.container} level="1">
      <AllSquares />
      <ConfettiCannon count={200} origin={{ x: 0, y: 0 }} autoStart={false} ref={confettiRef} />
    </Layout>
  )
}
